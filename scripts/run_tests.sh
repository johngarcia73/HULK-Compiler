...
run_ok_test() {
    ...
    local out_name="${cat//\//_}_$name"

    # Compile: run from student repo so output lands in results folder
    local compile_exit=0
    (cd "$STUDENT_REPO" && "$HULK" --output "$RESULTS_DIR/$out_name" --debug "$hulk_file" > /dev/null 2> "$tmperr") || compile_exit=$?
    ...
    actual=$(cd "$STUDENT_REPO" && "$RESULTS_DIR/$out_name" 2>/dev/null) || runtime_exit=$?
...
run_error_test() {
    ...
    local actual_exit=0
    local out_name="${cat//\//_}_$name"
    (cd "$STUDENT_REPO" && "$HULK" --output "$RESULTS_DIR/$out_name" --debug "$hulk_file" > /dev/null 2> "$tmperr") || actual_exit=$?
