...
2. [Compiler Architecture and Pipeline](#2-compiler-architecture-and-pipeline)
3. [Lexical Analysis and Lexer Generation](#3-lexical-analysis-and-lexer-generation)
4. [Syntactic Analysis and Parser Generation](#4-syntactic-analysis-and-parser-generation)
5. [Semantic Analysis and Inference Pipeline](#5-semantic-analysis-and-inference-pipeline)
6. [AST Lowering](#6-ast-lowering)
7. [IR Generation](#7-ir-generation)
...
1. The overall compiler architecture and full compilation pipeline
2. The design and automatic generation of the lexical analyzer
3. The design and automatic generation of the LALR(1) parser
4. Semantic analysis and type resolution
5. AST lowering and intermediate transformations
6. Intermediate Representation (IR) generation
7. The C runtime environment and execution

---

# 2. Compiler Architecture and Pipeline

## 2.1 Overview

The Hulk compiler follows a pipeline architecture that processes source code sequentially from raw text to an executable binary. The user's input source code is concatenated with a `builtin.hulk` file that defines core protocols and system functions. This combined code then flows through the compilation pipeline.

The compiler is broadly divided into a frontend (Lexing, Parsing, Semantics) and a backend (Lowering, IR Generation, Runtime).

### Main Components

- **Frontend Pipeline**: Orchestrates the first three compilation phases. Implements caching strategies to avoid recomputing analyses for grammars that have not changed.
- **Analyzer Generators**: 
...
- **Semantic Analyzer**: Performs type checking, name resolution, dependency analysis, and validation of the language's semantic constructs.
...
## 2.2 Data Flow and Error Handling

The compiler's data flow is unidirectional. The compiler implements a hierarchical error system:
...
## 2.3 Compilation Pipeline Integration
...
```mermaid
flowchart TD
    source["source.hulk + builtin.hulk"] --> frontend["Frontend Pipeline"]
    frontend --> ast["Semantically Validated AST"]
...
# 3. Lexical Analysis and Lexer Generation
...
## 3.3 Complete Lexer Generator Pipeline
...
## 3.4 Runtime Tokenization Algorithm
...
Register decimal with higher priority. The DFA will prefer the longest match thanks to continuing as long as valid transitions exist.
...
# 4. Syntactic Analysis and Parser Generation
...
## 4.2 Complete Parser Generator Pipeline
...
# 5. Semantic Analysis and Inference Pipeline
...
## 5.2 Complete Semantic Analysis Pipeline
...
## 6.2 Lambda Lifting to Delegates
...
## 6.3 Method Calls and Overloaded Function Calls
...
## 7.2 Name Management and String Pooling
...
## 7.3 Memory Management and Boehm GC
...
## 7.4 Object Layout and Type Information
...
## 7.5 Function Definitions and Method Tables
